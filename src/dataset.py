"""
Dataset module for Kaggle Alzheimer's dataset (preprocessed images).
"""
...
class AlzheimerMRIDataset(Dataset):
    """Dataset for Kaggle preprocessed Alzheimer's MRI images."""
    ...
    def __init__(
        ...
        print(f"[Kaggle] Loaded {len(self.samples)} images for {split}")
...
def get_data_loaders(
    ...
    """Create Kaggle data loaders."""
    train_dataset = AlzheimerMRIDataset(data_dir, split='train')
    test_dataset = AlzheimerMRIDataset(data_dir, split='test')
...
def get_sample_images(data_dir: str, num_samples: int = 4) -> Dict[str, List[str]]:
    """Get sample images for visualization."""
    dataset = AlzheimerMRIDataset(data_dir, split='train')
